...
router.get("/unread_summary", authToken, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ status: 500, message: "Failed to load unread summary" });
  }
});
...
router.post("/mark_direct_messages_read", authToken, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ status: 500, message: "Failed to clear DM unread" });
  }
});
...
router.post("/mark_channel_read", authToken, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ status: 500, message: "Failed to clear channel unread" });
  }
});
...
router.get("/unread_storage_mode", authToken, async (_req, res) => {
  try {
    ...
    console.error("Storage mode check error:", error);
    res
      .status(500)
      .json({ status: 500, message: "Failed to check storage mode" });
  }
});
